...
class Manager(object):
    ...
    def fit(self):
        ...
        #     max_entries=100000
        # )
        for iterations, ref, mov, metric in self.stages:
            ref.detach()
            mov.detach()
            ...
            for step in range(iterations):
                ...
                with torch.no_grad():
                    ...
                    if losses[step-self.convergence_win:step].diff().mean().abs() < self.convergence_val:
                        break
    ...
    def _makeresfun(ndim, smoothings, scales):
        ...
        return lambda *imgs: tuple(
                samplefn(
                    convfn(img, (tmp:=torch.ones((1,1,*smoothings), dtype=img.dtype, device=img.device))/tmp.sum()),
                    scale_factor=scales
                )
                for img in imgs
            )
